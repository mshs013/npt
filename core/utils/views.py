...
def dynamic_view(request, app_name, model_name, context):
    ...
    # Prepare context
    context.update({
        'object_list': [{'fields': [(field, get_display_value(obj, field)) for field in list_display], 'pk': obj.pk} for obj in object_list],
        'headers': headers,
        'app_name': app_name,
...
def dynamic_trashed_view(request, app_name, model_name, context):
    ...
    total_count = model.deleted_objects.count()  # Total count of all deleted rows
